...
total_capital = st.sidebar.number_input(
    ...
    value=10000.0, 
    step=100.0,
    help="Basis for fractional share calculations and wealth projections."
)

# FIXED UNIVERSE - AS PER COLAB RESEARCH
...
if st.button("🚀 Execute Full Institutional Analysis"):
    with st.spinner("Synchronizing with Market Data & Research Parameters..."):
        ...
        # 2. HRP OPTIMIZATION ENGINE
        hrp = HRPOpt(returns)
        weights_dict = hrp.optimize()
        weights = pd.Series(weights_dict).sort_values(ascending=False)
        
        # 3. PERFORMANCE METRICS (Sourced from Historical Returns)
        ...
        st.divider()
        st.write("### 📋 Trade Execution & Asset Allocation (Fractional)")
        ...
        with sc2:
            ...
            st.caption("Historical worst 5% daily move.")

        # 5. RISK VISUALS & HEATMAP
        t1, t2 = st.tabs(["Correlation Heatmap", "Return Distribution"])
        with t1:
        ...
        st.success("Institutional Terminal Synchronized.")
else:
    st.info("👈 System Standby. Configuration locked to research universe.")
